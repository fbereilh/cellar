...
- **Code, Markdown, SQL, chat, and raw cells**, with a run queue, live run status, and staleness tracking so you always know what's fresh. Mojo is not one of them: a notebook is written in Python or in Mojo as a whole, chosen at the top - see the Mojo bullet below. Adding a code cell picks up the kind of the code cell above it, so a run of SQL cells doesn't drop back to plain code every time you insert one (a cell you add as Markdown or Chat is what you asked for, and with no code cell above it you get an ordinary code cell). A **raw cell** is nbformat's type for verbatim text a downstream tool reads - Quarto or nbdev frontmatter, an nbconvert directive: Cellar never executes it and never renders it, so it carries no Run button at all and reaches the `.ipynb` exactly as you typed it. Switch a cell to it from the type label at the right of its toolbar, the command palette, or `r` in command mode. (A `.py` jupytext / Databricks-source notebook is rebuilt from its cells on every save and has no raw marker, so it cannot hold one: the option isn't offered there, and setting it is refused rather than lost on the next reload. The HTML export shows a raw cell as muted verbatim text, never as live HTML.)
- **Chat cells - ask Claude about the notebook, from inside it.** Add one with the **Chat** button at the bottom of the notebook or in the hover strip between two cells, or switch an existing cell to **Chat** from the type label at the right of its toolbar, the command palette, or `t` in command mode; then write a question and run it: Cellar sends the cells above it (their source and output text, minus any you have hidden from AI) along with your question, then streams the reply back into the cell as rendered markdown that is saved with the notebook. It runs through your Claude Code CLI login and never touches the kernel, so a chat cell answers even in a notebook that never started one. The reply is a fresh model answer every time, so re-running one costs a request and overwrites the answer you were reading - which is why **Run all** and an agent's batch runs skip chat cells and say so; run one deliberately from its own Run button. Choose which account answers in the sidebar's **Chat** section: it borrows the `claude` login you already have in your terminal, or you can sign into a separate Cellar-only account there and switch between them - Cellar can sign out of its own accounts and never of your terminal one. **Settings → Chat cells** holds the other choices, all per user rather than per project: which model replies (Sonnet unless you change it), **Learning mode**, and three opt-ins that are **off by default**, so out of the box a reply answers from the notebook alone with every tool disabled. **Learning mode** is off by default too, and changes how a reply is written rather than what it may reach: instead of just answering, the reply teaches - building the idea up from first principles, in short blocks, and checking your understanding as it goes. **Allow web search** lets a reply run web searches, which means queries derived from your notebook's content reach the search service; it is search only, never arbitrary URL fetching. **Allow reading workspace files** lets a reply read, glob and grep the files in the workspace to answer about your code: reads are confined to the workspace folder (paths outside it are refused, including through `..` or a symlink), the notebook you are chatting in is never readable as a file (the reply already has it as a fresher transcript, minus the cells you hid from AI) and neither are the copies Cellar writes from it or Cellar's own `.cellar` folder, and the other `.ipynb` notebooks in the workspace stay unreadable unless you also turn on **Allow reading other notebooks**. Reading is read-only throughout: a chat cell still cannot write or edit files, or run code. **When a reply uses a tool, the output says so**: each call adds one short, dimmed line naming the tool and what it was pointed at - the search query, or a file path written relative to the workspace rather than absolute - and marks a call that failed or was refused, so a claim you read is traceable to where it came from. What the tool *returned* is never shown, so a file's contents stay out of the notebook and out of an exported report. Those lines are saved and exported with the reply. (Needs the `claude` CLI on your `PATH`. A `.py` jupytext / Databricks-source notebook cannot hold a chat cell, for the same reason it cannot hold a raw one, so neither the **Chat** add buttons nor the menu option is offered there. Images in a *machine-written* markdown output never load themselves - a reply, or a kernel `display(Markdown(...))`, shows an image as its alt text or its URL - because a reply is written from notebook content that may not be yours; markdown cells you write are rendered exactly as before.)
- **Mojo notebooks.** A notebook is written in one language, chosen once in the **Language** bar at the top of it: **Python** (the default) or **Mojo**. Every plain code cell in that notebook is that language - Markdown, raw, SQL and chat cells are untouched by the choice, and so is the kernel: switching writes nothing into your cells and restarts nothing, so the Python kernel keeps its variables. A Python notebook records nothing for this at all (an absent setting *is* Python), so switching to Mojo and back leaves the file exactly as it was, and a notebook written before Cellar had the setting is a Python notebook with nothing to convert. Cellar runs a Mojo cell through Modular's own `%%mojo` cell magic on the notebook's Python kernel - Modular ships no Mojo Jupyter kernel, and that magic is the supported way to run Mojo in a notebook. The cell stores plain Mojo, so the `.ipynb` still opens anywhere. **Each Mojo cell is a separate program**: the magic writes it to a temp file and runs `mojo run` in a subprocess, so nothing carries from one Mojo cell to the next - no variables, no imports, no definitions - and every one needs its own `def main():`. That is Modular's model rather than a Cellar limit, and it is why a Mojo notebook's cells show no staleness and cannot be its imports cell: they hold no Python. Such a notebook exports to a `.mojo` module instead - see the export bullet below. A Mojo cell's output arrives when it finishes rather than streaming as it goes. Running one needs the Mojo toolchain in the project environment: run a Mojo cell without it and the cell itself tells you the exact command, `uv pip install max`. Cellar never installs it for you (~534 MB across 16 packages) - install it and run the cell again, with no kernel restart. (A `.py` jupytext / Databricks-source notebook is rebuilt from its cells on every save and stores no notebook metadata, so it cannot be a Mojo notebook: the Language bar isn't offered there, and asking for it - from an agent, say - is refused rather than lost on the next reload.)
...
- **Links in rendered content open in a new tab.** Clicking a link in a markdown cell, a chat reply, a `display(Markdown(...))` output or a widget's HTML never navigates your notebook away, so the kernel, the cells that are running and anything you have typed but not saved stay where they are; the page that opens gets no handle on the Cellar window. A link to a spot inside the same document (`#section`) still scrolls there in place, and Cellar's own buttons and links are unaffected.
- **Rich outputs**: matplotlib, Plotly, rich HTML, and images you can double-click to view at natural size. HTML tables that aren't read as a DataFrame - a table you render with `IPython.display.HTML`, one another library emits as rich HTML, or a `Styler` the grid turns down - get comfortable padding and alignment out of the box, so you don't need a `set_table_styles` helper on every table; anything you style yourself still wins. (Cellar states a few defaults directly on the header cells and the caption - their `color`, the caption's weight, and the index/caption alignment - so a *whole-table* rule such as `set_table_styles([{'selector': '', 'props': 'color:#444'}])` moves the data cells but not those; target them directly, e.g. `#T_xxxx th`, and yours wins.)
- **Interactive DataFrame grid**: pandas *and* polars frames become a sortable, filterable, paginated table instead of a static repr - both while the cell is live and when you reopen the notebook. A pandas `Styler` gets the grid too, keeping the values as you formatted them (`{:,.2f}` becomes `1,234.50`) and its `set_caption` title; what a grid structurally cannot express is the CSS, so a background gradient, per-cell colours or bars are dropped. Nested (MultiIndex) column headers are joined with ` / `. A table Cellar cannot read with confidence keeps its static HTML repr instead - a layout that doesn't match the frame's own declared shape, a very large `Styler`, or one whose cells hold markup such as a link, an image, or an inline sparkline - because a grid quietly showing the wrong columns is worse than no grid.
...
- **Export chosen cells to a module** (nbdev-style): name a target file in the **Export to** bar at the top of the notebook, mark the cells you want with the export toggle in each one's toolbar, and Cellar writes that module - a real, committable file holding just those cells, not a mirror of the whole notebook. **The module is written in the notebook's own language**, so the target's extension follows that rather than being a second setting that could disagree with it: a Python notebook exports a `.py`, a Mojo one a `.mojo`, switching the notebook's language re-expresses the target you named (`utils.py` becomes `utils.mojo`, and back), and a target whose extension names the other language is refused instead of stored. Cellar does not rename files behind you, so the module generated under the old extension stays on disk and the bar names it, for you to delete once nothing imports it. It is written when you **ask** for it and never by an ordinary save: the **Export** button (which names the kind of module once you have a target - **Export to .py**, **Export to .mojo**), naming the target, or marking or unmarking a cell. So editing a marked cell leaves the module exactly as it was until one of those - typing in a notebook never rewrites a git-tracked file behind you. Beside the target you choose what its path is measured from: the **workspace root** (the default), the **notebook folder**, so the module travels with the notebook if you move it, or the notebook's **git root**. Under either of the latter two the bar echoes the workspace file the path resolves to, and it says so when a target cannot resolve at all rather than silently generating nothing. If the notebook has a code root and a `.py` module would land outside it, the bar warns you, because that is the case where the export succeeds and the notebook's kernel still cannot `import` it; a `.mojo` target is not warned about, since a Mojo cell reaches the compiler through a subprocess rather than the kernel's import path, so where the module sits changes nothing. It only ever overwrites files it generated, so aiming the target at a module you wrote by hand is refused instead of clobbering it, and unmarking the last cell leaves the module on disk exactly as it was. Marks are never erased behind you: convert a marked cell to a type that contributes no module source, or paste Mojo carrying a `%%mojo` line into a marked cell of a Python notebook, and the cell keeps its mark, shows it greyed and labelled **not exported**, and one line in the export bar says how many there are and what to do - clearing one is a click on its greyed toggle. For a `.py` target it also tells you when the module those marks describe is one Python refuses to run: a cell that keeps a `from __future__ import ...` on the same line as another statement, separated by a semicolon, makes an uncompilable module, and Cellar cannot move that line without reordering the statement riding with it - so the export bar warns you as soon as a marked cell holds one, before you export, naming the line and the one edit that fixes it; an export you then run still writes the module and repeats the warning rather than reporting a clean export. That warning names a problem it found; it is not a promise that the module compiles. **A `.mojo` module carries at most one `def main()`**, because two in one Mojo file is a hard compile error and Modular's `%%mojo` magic runs each cell as a whole program, so nearly every Mojo cell has one: the **last** exported cell that carries a top-level `main` keeps it, and every earlier one loses that block. Nothing goes silently: each affected cell says **main not exported** in its toolbar, the generated module carries a comment where each block was removed, and the export bar names them all in one line. Export no main-carrying cell and you get a plain library module with none, and no warning at all. A module that does keep one is both importable from Mojo and runnable on its own (`mojo run`, `mojo build`), and says in its own header that Python cannot import it - a Python import builds it as a shared library, which refuses a module defining `main`. Your agent can drive all of it - the target, the base it is measured from, and which cells - so "pull these helpers into `lib/utils.py`" is one instruction.
- **Coexists with nbdev.** Cellar reads nbdev's own cell directives, so a notebook written for nbdev needs no re-marking: a bare `#| export` in the directive lines at the top of a Python code cell marks that cell for export, and a `#|default_exp` names the module those cells go to - resolved under the project's `lib_path`, where nbdev itself puts it, rather than relative to the workspace root as it is outside an nbdev project. Only a plain `#| export` counts, and only in that leading block, which is nbdev's own rule; `exporti`, `exports`, `exportd` and a `#| export <other-module>` are left alone, so a cell carrying one lands in the module only if you also mark it with the toggle. Cellar never writes a `#|` line into your code, so a cell marked that way cannot be unmarked from Cellar: its export toggle stays on and tells you which line to delete, instead of reporting a change the notebook did not take. And because Cellar only ever overwrites modules it generated itself, opening an established nbdev project writes nothing into its library - it reads the marks and the target correctly and says the file at that target is not one it wrote. If the library lives outside the folder you opened Cellar in - the usual layout when you open `nbs/` - the target is refused, naming both ways out (open Cellar at the project root, or set an explicit target), rather than leaving a stray module beside your notebooks. nbdev's cleanup (`nbdev-clean`, and the Jupyter save hook `nbdev-install-hooks` installs) rebuilds a notebook's metadata from an allowlist and drops everything else, so in an nbdev project it silently erases the settings Cellar keeps there - your export target, report view, header numbering and per-cell export marks. Open Cellar in one and the sidebar says so, names the `pyproject.toml` it means, and offers nbdev's own supported fix: `allowed_metadata_keys` and `allowed_cell_metadata_keys` naming `cellar` under `[tool.nbdev]`. Nothing is written until you click - the file is yours - and the write merges, keeping any values those keys already hold and leaving the rest of the file byte-for-byte. A shape Cellar will not edit confidently (an inline table, a value that isn't a plain list) is refused with the two lines to paste in yourself. The card appears only when it applies, and is gone once the keys are there.
- **Checkpoints and undo** for agent actions - snapshot before a risky change and roll back.
- **Tab completion and `Shift-Tab` docs from the live kernel.** In a Python code cell, `Tab` completes against what is actually alive in the kernel - a variable a cell created when it ran, a module imported three cells up, the attributes and columns of a live object - not only what can be read out of the file. It is added *beside* the editor's own completion rather than replacing it, so a name you are typing in a cell you have not run yet still completes, and with no kernel at all the editor behaves exactly as it did before. `Shift-Tab` with the caret inside a call's arguments opens that callable's signature and docstring; press it again to expand to the full documentation, source included, and `Escape` (or moving the caret, editing, or clicking away) dismisses it - classic Jupyter's contract, answered by the kernel's own introspection. Neither key ever waits behind a running cell, and neither ever starts a kernel: while a cell is running `Shift-Tab` says so rather than blocking, and completion quietly falls back to the editor's own suggestions. `Tab` on an empty line still moves focus out of the editor as it always did. Both are listed under **Settings → Keyboard shortcuts** and rebindable like every other one. (Python code cells only - Markdown, raw, SQL and chat cells, and every cell of a Mojo notebook, keep `Tab` and `Shift-Tab` as they were, since the kernel would answer about Python.)
- **`name?` and `name??` documentation, right in the cell.** End a line with `?`, run the cell, and it shows that object's documentation - its signature and docstring; `??` adds its source. It works for whatever you can name in the kernel: a function, a class, a module, or a live object you just built. IPython answers this itself, so a name that does not exist gets IPython's own plain-text "Object `foo` not found." reply rather than an empty box. The answer arrives as ordinary cell output, which is what makes a long one usable: it stays after you click away, scroll off, or reload the notebook, it scrolls inside the cell like any other tall output, and you can select it or take it with the cell's copy-output button. `json.dumps??` comes across whole - nothing is trimmed quietly, and if an output is long enough to hit Cellar's per-cell cap you see the truncation marker every output gets. Terminal colour codes are stripped before the text is saved, so the `.ipynb` still reads cleanly in Jupyter or on GitHub. (Python code cells only, again because IPython is the one answering: a Markdown, raw, SQL or chat cell - and any cell of a Mojo notebook - never reaches it as Python, so a `?` in one is left alone.)
...
- *Optional*: the **Mojo toolchain** in the project venv (`uv pip install max`) - only Mojo notebooks use it, and only when you run a cell in one
...
- **Unit tests** (`tests/unit/`) guard the pure server logic. The crown jewel is clean-on-save: idempotent, git-clean round-trips, the metadata allowlist, memory-address scrubbing, and the notebook model (stable cell IDs, add/move/delete, duplicate-ID re-keying). These are the fast half of the gate and **run on every PR in CI**.
- **E2E** (`tests/e2e/`) drives the real `cellar` launcher against a scratch workspace in a browser. The smoke spec (`smoke.spec.ts`) runs `6*7`, asserts `42` renders, and confirms the saved `.ipynb` is valid; the rest cover behavior only the full stack can show (e.g. `kernel-watchdog-probe.spec.ts` proves a long, silent cell is never aborted for being silent). They need the full kernel runtime (`uv` + `python3` + the cached host-venv), which they skip themselves without - so install the browser once with `npx playwright install chromium`. `npm run test:e2e` rebuilds the app first when `build/` is stale, absent or incomplete (the specs serve the production build, so a stale one would silently test uncompiled code) and runs two spec files at a time: **~9.8 min for the full 476-test suite** locally. **They also gate every PR**, sharded across ten runners so the check lands in about **5 minutes** rather than the ~45-60 min an unsharded serial run would cost (`.github/workflows/e2e.yml`, which also fails loudly rather than skipping when the runtime is missing - a fully-skipped Playwright run exits 0).
