...
- **Mojo cells.** Switch a cell to **Mojo** from the type label at the right of its toolbar, and Cellar runs its Mojo source through Modular's own `%%mojo` cell magic on the notebook's Python kernel - Modular ships no Mojo Jupyter kernel, and that magic is the supported way to run Mojo in a notebook. The cell stores plain Mojo, so the `.ipynb` still opens anywhere. **Each Mojo cell is a separate program**: the magic writes it to a temp file and runs `mojo run` in a subprocess, so nothing carries from one Mojo cell to the next - no variables, no imports, no definitions - and every one needs its own `def main():`. That is Modular's model rather than a Cellar limit, and it is why a Mojo cell shows no staleness, cannot be the imports cell, and has no place in a `.py` module: it holds no Python. Mojo cells export to a `.mojo` module of their own instead - see the export bullet below. Its output arrives when the cell finishes rather than streaming as it goes. It needs the Mojo toolchain in the project environment: run a Mojo cell without it and the cell itself tells you the exact command, `uv pip install max`. Cellar never installs it for you (~534 MB across 16 packages) - install it and run the cell again, with no kernel restart. (A `.py` jupytext / Databricks-source notebook cannot hold a Mojo cell, for the same reason it cannot hold a raw or a chat one.)
