 ...
 * is added to the worktree's own `.git/info/exclude`, so the user's checkout does
 * not show it as an untracked change and it can never be committed onto the
 * branch under review. That is asserted through REAL `git status`, because it is
 * a claim about what git does, not about what we wrote.
 */
...
beforeEach(() => {
	...
	const excl = join(WS, '.git', 'worktrees', 'pr-398', 'info', 'exclude');
	if (existsSync(excl)) rmSync(excl, { force: true });
});
...
describe('what gets written into an adopted worktree', () => {
	...
	it('leaves the checkout CLEAN, via the worktree’s own .git/info/exclude', () => {
		mod.configureAdoptedWorktree(SIBLING);
		// The mitigation that makes on-by-default defensible: git itself must not see
		// the file. Asserted through real `git status`, not by reading back what we
		// appended — the claim is about git's behavior.
		expect(git(SIBLING, 'status', '--porcelain', '-uall').trim()).toBe('');
		// Per-worktree, so the MAIN checkout is untouched and no teammate ever sees it.
		expect(git(WS, 'status', '--porcelain', '-uall')).not.toContain('info/exclude');
		expect(readFileSync(join(WS, '.git', 'worktrees', 'pr-398', 'info', 'exclude'), 'utf8')).toContain('.mcp.json');
	});

	it('is IDEMPOTENT: a second adoption writes zero bytes and adds no duplicate line', () => {
		mod.configureAdoptedWorktree(SIBLING);
		const excludePath = join(WS, '.git', 'worktrees', 'pr-398', 'info', 'exclude');
		const firstExclude = readFileSync(excludePath, 'utf8');
		...
		expect(readFileSync(excludePath, 'utf8')).toBe(firstExclude);
	});
});
