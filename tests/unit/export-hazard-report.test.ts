...
/**
 * A hazard describes the module these marks make. Over a target the clobber guard
 * declines there is no module of ours and never will be - MCP's `moduleHazard` says
 * outright that it WAS written - so the foreign-module question is asked FIRST, and
 * it is asked in ONE place (`docExportHazards`), which is what stops the export bar
 * and MCP from describing one document differently.
 *
 * Newly reachable: reading nbdev's `#| export` means every notebook in an
 * established nbdev repo HAS marks, so every explicit export there reaches the
 * module write, and that module carries nbdev's OWN header - so the guard declines
 * there routinely.
 */
describe('a module Cellar did NOT write cannot carry a hazard about it', () => {
	...
	it('still reports the hazard over a module Cellar generated, and over none at all', async () => {
		...
		// Marking is one of the explicit export actions, so OUR module is already on
		// disk holding the hazard - the case the warning is FOR.
		const res = nbmod.exportPy(target);
		expect(res.written || res.reason === 'unchanged').toBe(true);
