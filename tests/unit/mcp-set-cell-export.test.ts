...
describe('a regeneration that FAILED is reported, never read as a success', () => {
	...
	it('refuses to overwrite a .py file it did not generate, and reports why', async () => {
		...
		expect(r.ok ? r.module : null).toMatchObject({ regenerated: false });
		// It names the file and BOTH remedies, and specifically NOT the "edit a marked
		// cell" advice the sibling branch gives - re-entering this refusal forever is
		// the one thing an agent must not be told to do.
		const reason = r.ok ? (r.module?.reason ?? '') : '';
		expect(reason).toMatch(/handwritten\.py was not generated by Cellar/);
		expect(reason).toMatch(/delete that file/);
		...
		expect(nbmod.exportPy(target)).toMatchObject({
			written: false,
			reason: 'foreign-module',
			target: 'handwritten.py'
		});
		expect(nbmod.lastExportError(target)).toBe(null);
		expect(readFileSync(join(WS, 'handwritten.py'), 'utf8')).toBe(handwritten);
		...
		expect(nbmod.exportPy(target)).toMatchObject({ written: false, reason: 'unchanged' });
	});

	/**
	 * The PAIR reports it too. `set_export_target` persists and therefore triggers the
	 * same regeneration, so pointing a target AT nbdev's own module runs the same
	 * refusal - and since that refusal is an OUTCOME rather than a throw it sets no
	 * `lastExportError`, so a tool that does not ask emits no `module` field at all,
	 * which under the conditional contract says the module WAS regenerated. Silent
	 * success is exactly what the field exists to prevent, so the two tools must
	 * describe one document identically.
	 */
	it('reports the same refusal when the TARGET is pointed at a foreign module', async () => {
		const { target, code } = await makeNotebook('target-foreign.ipynb');
		const theirs = '# AUTOGENERATED BY NBDEV! DO NOT EDIT!\ndef theirs():\n    return 1\n';
		writeFileSync(join(WS, 'theirs.py'), theirs);
		// The marked cell also carries a compile HAZARD, which is the worse half: with
		// no foreign check the hazard branch answers `regenerated: true` and
		// "<target> was written, but ...", an outright claim of a write that never
		// landed - so the refusal has to be asked BEFORE it.
		nbmod.setSource(svc.resolveRef(target, code[0]), 'from __future__ import annotations; x = 1', target);
		svc.setCellExport([code[0]], true, target);

		const r = svc.setExportTarget('theirs.py', target) as {
			export_target?: string;
			module?: { regenerated: boolean; reason?: string; warning?: string };
		};
		expect(r.export_target).toBe('theirs.py');
		expect(r.module?.regenerated).toBe(false);
		// The SAME sentence the per-cell tool gives - one helper, not a second copy.
		expect(r.module?.reason).toMatch(/theirs\.py was not generated by Cellar/);
		// ...and never the hazard's "was written" claim, over a write that never landed.
		expect(r.module?.warning).toBeUndefined();
		expect(readFileSync(join(WS, 'theirs.py'), 'utf8')).toBe(theirs);
		expect(nbmod.lastExportError(target)).toBe(null);

		// Unchanged where nothing is foreign: a free path really writes, and the hazard
		// is reported there - as a WARNING about a module that exists, which is the
		// claim the foreign case may not make.
		const free = svc.setExportTarget('lib/free.py', target) as {
			module?: { regenerated: boolean; warning?: string };
		};
		expect(readFileSync(join(WS, 'lib/free.py'), 'utf8')).toContain('__future__');
		expect(free.module?.regenerated).toBe(true);
		expect(free.module?.warning).toMatch(/lib\/free\.py was written/);
		// ...and re-pointing at the module Cellar just generated says the same thing.
		expect(
			(svc.setExportTarget('lib/free.py', target) as { module?: { regenerated: boolean } }).module?.regenerated
		).toBe(true);
	});

	/**
	 * The gate that makes it a REFUSAL rather than a description of a file nobody
	 * touched: with nothing marked, `exportNotebookToPy` returns `no-cells` before it
	 * reaches the write, so the guard never declined and neither tool may say the
	 * file was "left untouched" by an attempt that did not happen. Naming a target
	 * before marking anything is the normal first step of the export flow.
	 */
	it('says nothing about a foreign file when no cell is marked', async () => {
		const { target } = await makeNotebook('target-foreign-unmarked.ipynb');
		writeFileSync(join(WS, 'untouched.py'), 'def theirs():\n    return 1\n');
		const r = svc.setExportTarget('untouched.py', target) as { module?: unknown };
	...
	it('a failed manual export records why, so a later idempotent call still reports it', async () => {
		...
		expect(nbmod.lastExportError(target)).toBeTruthy();

		const again = svc.setCellExport([code[0]], true, target);
		expect(again.ok ? again.module : null).toMatchObject({ regenerated: false });
		expect(again.ok ? again.module?.reason : '').toMatch(/could not be written/);
	});
});
