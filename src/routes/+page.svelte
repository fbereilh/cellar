<script lang="ts">
	...
	async function exportPy() {
		...
		// error, so no error channel carries it.
		else if (r.reason === 'foreign-module')
			showNotice(`${r.target} was not generated by Cellar, so it was left untouched - point the export target elsewhere, or delete that file.`);
		else {
