/**
 * The DOM half of a token chip: put `token` into an affix INPUT at its caret.
 *
 ...
 * pane's defaults were carrying as two line-for-line copies. Both writes matter:
 * the inputs are `value=`-bound rather than two-way, so Svelte will not re-render an
 * element it already agrees with, and without restoring the selection a second chip
 * click would append past the first insertion instead of continuing where the user
 * was.
 *
