...
export function registerTools(server: McpServer) {
	...
	server.registerTool('databricks_runtime', { description: 'Advertise a Databricks runtime, applied to YOUR working notebook\'s kernel. The preference is per WORKSPACE: it also governs what every other connected notebook advertises at its next kernel start, though only this one restarts now. It sets DATABRICKS_RUNTIME_VERSION, the variable notebook code\'s IS_DATABRICKS = os.getenv("DATABRICKS_RUNTIME_VERSION") is not None gate reads at import time. True routes parameter handling to dbutils.widgets (real ipywidgets Cellar renders) instead of the local CLI parser (tyro/argparse over sys.argv) that branch falls back to and that cannot work inside a kernel. It is not only your own gate: mlflow\'s is_in_databricks_runtime() reads the same variable, which is why this is opt-in and off by default. Applying it RESTARTS the kernel and CLEARS every variable - kernel_restarted/namespace_cleared say whether that happened, so re-run your cells when true. It restarts ONLY when the state changes: re-enabling what is already advertised is a no-op, and with no kernel the preference applies at the next start. On a notebook with NO Databricks session it stores the preference and advertises nothing (the injection is scoped to a connected one) - connect first, then call again. `version` sets the advertised string (default a recent LTS line), never checked against a real runtime. Read the current state from the runtime block on databricks_status / kernel_state / get_notebook_map: advertised:false is why dbutils.widgets code takes its non-Databricks branch. Errors: runtime_env_forced (CELLAR_DATABRICKS_RUNTIME holds the decision; nothing here can move it), runtime_restart_failed (it carries the same pair - namespace_cleared:true there still means your variables are gone).', inputSchema: { enable: z.boolean(), version: z.string().optional(), ...notebookParam } }, async ({ enable, version, notebook }, extra: ToolExtra) => databricksTool(() => withProgress(extra, () => svc.databricks.setRuntime({ enable, version, nb: targetOf(extra, notebook) }))));
