...
/**
 * The `.py` text-notebook refusal both halves of the export flow share
 * (`set_cell_export`, `set_export_target`), naming the CAUSE rather than reading
 * as a missing cell or an unexplained failure. One message, one clause per tool
 * for what specifically cannot be done, so the pair cannot drift about why.
 */
 ...
 * while this is about it being REBUILT FROM ITS CELLS, which is what drops those
 * types. The rule and its messages live once, in `$lib/cellLanguage` - the
 * writers throw it (`assertCanHoldType`) and `addCells` raises it for a whole
 ...
 * one rule rather than each re-deriving when such a cell is allowed.
 */
async function unsupportedTypeRefusable<T extends { content: unknown[] }>(fn: () => Promise<T>) {
	try {
		return await fn();
	} catch (err) {
		if (err instanceof TextNotebookCellTypeError) return notFound(err.message);
		throw err;
	}
}
...
async function databricksTool(fn: () => unknown) {
	try {
		...
		// It may also carry `kernelRestarted` - a side effect already PAID before the
		// failure - which rides out as the same `kernel_restarted`/`namespace_cleared`
		// pair a success reports, so a failure can never hide a cleared namespace.
		const e = err as { code?: string; message?: string; kernelRestarted?: boolean };
		return { content: [{ type: 'text' as const, text: JSON.stringify({ error: e?.code ?? 'error', message: String(e?.message ?? err), ...(typeof e?.kernelRestarted === 'boolean' ? { kernel_restarted: e.kernelRestarted, namespace_cleared: e.kernelRestarted } : {}) }) }], isError: true };
	}
}
...
3. KERNEL_STATE IS THE LIVE TRUTH; SAVED OUTPUTS ARE NOT. A cell's outputs are
   ...
   CHAT CELLS ARE THE EXCEPTION TO "RE-RUN IT". A chat cell holds a question and
   its saved AI reply; it runs no kernel, so it answers in its own run_status
   words (ok_chat_reply / error_chat_reply = this server ran it,
   chat_reply_persisted = a saved reply) and none of them says anything about the
   namespace - there is nothing to refresh. Re-running one asks the model again:
   a billed, nondeterministic turn that overwrites the reply the user may be
   reading. So never re-run a chat cell to bring state up to date; run one only
   when the user asks for a new answer. (run_cells / run_all / run_range skip
   them outright for the same reason.)
...
6. DECLARE YOUR WORKING NOTEBOOK FIRST. Your read/write/run tools target YOUR
   ...
   where your edits land. A jupytext / Databricks-source .py is a notebook too and
   is listed and pinned AS-IS: pass its path with the .py extension, or you mint
   and pin a stray empty .ipynb instead (create is the default). If you never
   declare a working notebook, your tools fall back to the user's active tab, so
   your target moves when the user switches tabs. To reach into ANOTHER notebook
   for a single operation without changing your working notebook, pass that tool's
   optional notebook parameter (a workspace notebook path).
...
8. ONE KERNEL PER NOTEBOOK — ISOLATED NAMESPACES, PARALLEL EXECUTION. Each
   ...
   your kernel tools act on YOUR working notebook and nothing else. Concretely:
     ...
     - Runs serialize only within a notebook. Your notebook's kernel runs one cell
       ...
       An interrupt ends the run and reports HOW in its 'stopped' field: "forced"/"forced_no_signal" mean
       the kernel never answered (or was never reached), so it may still be executing that code even
       though your notebook is free again — say so rather than reporting a
       clean stop.
     ...
     - A notebook may declare a CODE ROOT: a directory its kernel runs in and
       imports from — normally a git worktree, either inside the workspace or a
       sibling OUTSIDE it — so two notebooks can run two checkouts of the same repo
       side by side. Most have none and run at the workspace root —
       get_notebook_map reports yours as "root" (null = the workspace), list_roots
       shows what is available, and use_notebook(name, root) sets it — or
       use_notebook(root) alone re-roots the notebook you already pinned — which
       frees that kernel, clearing its namespace. A root outside the workspace is
       accepted ONLY when it is a registered worktree of the workspace's own repo
       (list_roots marks those external:true); anything else is refused. Only the
       kernel's cwd and sys.path follow a root: the file tree, git, checkpoints and
       the interpreter stay workspace-wide, so a file path you read or write is
       still workspace-relative and CANNOT reach inside an external worktree. The
       one thing SETTING an external root does write there is Cellar's own agent
       config, so an agent whose cwd is that checkout can reach this instance; it
       is git-excluded, never committed, reported back on agent_config, and a
       Settings toggle turns it off. Do not create roots yourself — the user makes
       them.
...
9. DATABRICKS: USE THE SESSION, DO NOT BUILD ONE. kernel_state and
   ...
   CONNECTED IS NOT THE SAME AS "ON DATABRICKS". The same block carries
   runtime:{advertised, version, forced_by_env} - whether this kernel sets
   DATABRICKS_RUNTIME_VERSION, i.e. whether the notebook's own
   IS_DATABRICKS gate reads true. Connecting a cluster deliberately does NOT set it.
   So if a notebook's parameter code is gated on IS_DATABRICKS (the dbutils.widgets
   path) and runtime.advertised is false, that gate is the reason it is taking its
   CLI branch - say so, and offer databricks_runtime(enable:true), which restarts the
   kernel and clears the namespace. Do not "fix" such a notebook by rewriting its
   parameter code.
...
11. RAW CELLS ARE VERBATIM TEXT. cell_type "raw" is the nbformat raw type: Cellar
   never executes it and never renders it — it is passed through to the .ipynb
...
12. EVERY MOJO CELL IS A SEPARATE PROGRAM. cell_type "mojo" holds Mojo source.
   Cellar compiles it to Modular's own "%%mojo" cell magic, which writes the body
   to a temp file and runs \`mojo run\` in a SUBPROCESS — there is no Mojo kernel,
   and NOTHING carries from one Mojo cell to the next: no variables, no imports,
   no structs, no functions. So each mojo cell MUST be a complete program with its
...
export function registerTools(server: McpServer) {
	...
	server.registerTool('interrupt_kernel', { description: 'Interrupt YOUR working notebook\'s running kernel and drop its queued runs (stop means stop). Ends the run and reports HOW via `stopped`. "kernel" = it surrendered, the normal case. "forced" = it was asked and did not surrender in the grace window; "forced_no_signal" = the interrupt could not even be delivered, so it was never asked. For BOTH, Cellar stopped waiting and freed the notebook - the namespace is intact and your cells run again, but THE KERNEL MAY STILL BE EXECUTING that code, so relay that and use restart_kernel if you need certainty. Also "chat", "idle" (nothing was running) and "no_kernel". Affects only your notebook\'s kernel; other notebooks are untouched.', inputSchema: { ...notebookParam } }, async ({ notebook }, extra: ToolExtra) => text(await svc.kernel.interrupt(targetOf(extra, notebook))));
	...
	server.registerTool('list_notebooks', { description: 'List the workspace\'s notebooks: every .ipynb plus every jupytext/Databricks-source .py (workspace-relative, .py extension included - pass one to use_notebook exactly as listed). Each entry is flagged `working` (THIS session\'s — where your edits land) and `active` (the one the USER is looking at); they differ whenever you pinned your own. Set yours with use_notebook.', inputSchema: {} }, async (_args, extra: ToolExtra) => text(svc.listNotebooks(extra?.sessionId)));
	server.registerTool('use_notebook', { description: 'DECLARE this session\'s working notebook (a workspace .ipynb, or a jupytext/Databricks-source .py) — OPEN-OR-CREATE: opens the named notebook, or creates it if it does not exist (omit name for an untitled one). Pass a .py AS-IS, extension included: stripping it MINTS and pins a stray empty .ipynb instead (create is the default), and a .py can be neither created nor given a `root`. From then on your read/write/run tools default to it, independent of which tab the USER focuses and of what any OTHER agent is doing, and without stealing the user\'s focus (it is surfaced as an available tab). Call this FIRST when you start working. Pass create_if_missing:false for open-only (error if it does not exist). Optional `root`: a directory (see list_roots) this notebook\'s KERNEL runs in and imports from — workspace-relative, or a registered git worktree of this repo outside the workspace (absolute or ../-relative; stored ../-relative) — point it at a specific checkout; "" clears it back to the workspace root, omitting it leaves it alone. Setting an EXTERNAL worktree root also writes Cellar\'s agent config into that checkout, so an agent working there can reach this instance — git-excluded and never committed, reported on the result as `agent_config`, and turned off by a Settings toggle. `root` WITHOUT `name` re-roots the notebook you already pinned (it never creates one). To reach another notebook just once, pass that tool\'s `notebook` param instead.', inputSchema: { name: z.string().optional(), create_if_missing: z.boolean().optional(), root: z.string().optional() } }, async ({ name, create_if_missing, root }, extra: ToolExtra) => {
		try {
			// A root is REFUSED FIRST, before the notebook is opened/created and the
			...
			// pinned behind its own error. The name rides along because a `.py`
			// notebook cannot hold a root either, and that refusal comes from the
			// same write path - so it has to be raised here for the same reason.
			if (root !== undefined) svc.assertRootUsable(root, name);
	...
	server.registerTool('list_roots', { description: 'List this workspace\'s CODE ROOTS: directories a notebook\'s kernel can be rooted at — those under the conventional `roots/`, any a notebook already declares, and every registered git worktree of this repo (siblings outside the workspace included, marked external:true). Each carries the branch/commit checked out there and which notebooks point at it. A root changes only where a kernel runs and imports from — the file tree, git, checkpoints and the interpreter stay workspace-wide, and an external root grants no file access; the one thing ADOPTING one writes there is Cellar\'s agent config, so an agent working in that checkout can reach this instance (git-excluded, never committed; a Settings toggle turns it off). Declare one with use_notebook(name, root). An empty list means every notebook runs at the workspace root.', inputSchema: {} }, async (_args, extra: ToolExtra) => text(await svc.listRoots(extra?.sessionId)));
	...
	// --- write ---
	server.registerTool('add_cell', { description: `Add a code|sql|mojo|markdown|raw cell (optionally after a cell) with optional source. A sql cell holds a SQL query that runs against the connected Databricks spark session (doctrine clause 10). A mojo cell holds Mojo source, compiled to a %%mojo magic and run by \`mojo run\` — each one is a SEPARATE program (doctrine clause 12). A raw cell is verbatim text Cellar never executes or renders (Quarto/nbdev frontmatter, nbconvert directives) — see doctrine clause 11. Adds ONLY — it does not run or render, so a markdown cell added this way stays raw source; prefer add_and_run for markdown/sql.${ROUTE_IMPORTS_PTR}`, inputSchema: { after_id: z.string().optional(), cell_type: z.enum(['code', 'sql', 'mojo', 'markdown', 'raw']).optional(), source: z.string().optional(), route_imports: z.boolean().optional(), ...notebookParam } }, async ({ after_id, cell_type, source, route_imports, notebook }, extra: ToolExtra) => {
		const target = targetOf(extra, notebook);
		let after = after_id;
		if (after_id != null) { const res = resolveOne(target, after_id, extra?.sessionId); if ('error' in res) return res.error; after = res.id; }
		return unsupportedTypeRefusable(async () => {
	...
	server.registerTool('add_cells', { description: `Add multiple cells in order (optionally after a cell).${ROUTE_IMPORTS_PTR}`, inputSchema: { cells: z.array(z.object({ cell_type: z.enum(['code', 'sql', 'mojo', 'markdown', 'raw']).optional(), source: z.string().optional() })), after_id: z.string().optional(), route_imports: z.boolean().optional(), ...notebookParam } }, async ({ cells, after_id, route_imports, notebook }, extra: ToolExtra) => { const target = targetOf(extra, notebook); let after = after_id; if (after_id != null) { const res = resolveOne(target, after_id, extra?.sessionId); if ('error' in res) return res.error; after = res.id; } return unsupportedTypeRefusable(async () => text(await svc.addCells(cells, after, { routeImports: route_imports ?? true, nb: target }))); });
	...
	server.registerTool('consolidate_imports', { description: 'Sweep every MODULE-LEVEL import in your working notebook into its single "imports" cell (deduplicated, __future__ first, canonically ordered), strip those lines from the cells they came from, and run the imports cell so the kernel has them. Imports nested in a def/class/if/try body are left alone (lazy loading, TYPE_CHECKING - a choice, not an accident). Idempotent: a second run changes nothing and re-runs nothing. A cell the sweep EMPTIES is deleted; if it had outputs the sweep first checkpoints them, never throttled, so undo brings that cell back WITH them - and REFUSES before sweeping if it cannot; allow_unrecoverable:true proceeds knowingly.', inputSchema: { allow_unrecoverable: z.boolean().optional(), ...notebookParam } }, async ({ allow_unrecoverable, notebook }, extra: ToolExtra) => {
		const r = await svc.consolidate(targetOf(extra, notebook), { allowUnrecoverable: allow_unrecoverable === true });
		// The one refusal this tool can make is the shared output-unrecoverable one,
		// which carries its own cause and remedy; everything else is a result.
		return 'refused' in r ? notFound(r.reason) : text(r);
	});
	...
	server.registerTool('delete_cells', { description: 'Delete ONE OR SEVERAL cells by handle in one call (a single-element ids array deletes one cell). Nothing is deleted unless every id resolves, and the whole batch is one checkpoint, never throttled away - undo restores the cells WITH their outputs. If it cannot store them the call is REFUSED before deleting anything; allow_unrecoverable:true proceeds knowingly. Deleting EVERY cell is refused - a notebook always keeps at least one. Returns {ok, deleted:[ids], count, undo?}.', inputSchema: { ids: z.array(z.string()), allow_unrecoverable: z.boolean().optional(), ...notebookParam } }, async ({ ids, allow_unrecoverable, notebook }, extra: ToolExtra) => {
		const target = targetOf(extra, notebook);
		const res = resolveMany(target, ids, extra?.sessionId);
		if ('error' in res) return res.error;
		const r = svc.removeCells(res.ids, target, { allowUnrecoverable: allow_unrecoverable === true });
		if (r.ok) return text(r);
		// Two refusals share the field and must not share a message: one is the
		// notebook-always-keeps-a-cell invariant, the other is a checkpoint that could
		// not hold the outputs, which carries its own cause and remedy.
		if ('refused' in r && r.refused === 'outputs_unrecoverable') return notFound(r.reason);
		if ('refused' in r) return notFound('refused: a notebook always keeps at least one cell - deleting all of them is not allowed. Delete all but one, or replace the last cell\'s source instead.');
		return notFound(r.missing ? `cell ${r.missing} not found` : 'ids must not be empty');
	});
	server.registerTool('clear_outputs', { description: 'Clear the saved outputs of ONE OR SEVERAL cells by handle; OMIT ids to clear EVERY cell (an empty ids array is an error, never a clear-all). Nothing is cleared unless every id resolves. A pre-clear checkpoint always stores the outputs, so undo restores them; if it cannot the call is REFUSED before clearing - allow_unrecoverable:true proceeds knowingly. Display only: run_status/ran_this_session/staleness unchanged (they read the run stamp, not outputs); only has_output flips. A cell with no outputs is a no-op; one running is SKIPPED (a live run would outrun the report), named in `skipped`. Returns {ok, cleared:[ids] (those that HAD outputs), count, skipped?, undo?}.', inputSchema: { ids: z.array(z.string()).optional(), allow_unrecoverable: z.boolean().optional(), ...notebookParam } }, async ({ ids, allow_unrecoverable, notebook }, extra: ToolExtra) => {
		const target = targetOf(extra, notebook);
		let full = ids;
		...
		const r = svc.clearOutputs(full, target, { allowUnrecoverable: allow_unrecoverable === true });
		if (r.ok) return text(r);
		if ('refused' in r) return notFound(r.reason);
		return notFound(r.missing ? `cell ${r.missing} not found` : 'ids must not be empty - omit ids entirely to clear every cell');
	});
	...
	// The four destructive tools each state the same four facts - the checkpoint stores
	// the outputs, it is never throttled away, the call is REFUSED if it cannot, and
	// `allow_unrecoverable:true` proceeds knowingly - and each stays inside the 700-char
	// bound every session is billed for. This one's clause is the terse one because its
	// pre-existing content leaves under 100 chars of headroom: compress the wording
	// further if it must grow, never drop one of the four facts to buy the number.
	server.registerTool('set_cell_type', { description: 'Set a cell type to code, sql, mojo, markdown, or raw. sql tags the code cell as a SQL query (runs against the connected Databricks spark session); mojo tags it as Mojo (doctrine clause 12); code reverts it to Python. raw is verbatim text (frontmatter/directives): never executed, never rendered. raw, mojo and chat are all refused on a .py text notebook, which cannot store the tag. Converting away from code drops that cell\'s outputs and imports role; an unthrottled checkpoint stores them for undo, else REFUSED (allow_unrecoverable:true proceeds). Its nbdev export mark is KEPT: a mark the new type cannot honour strands rather than being silently deleted, and is cleared from the cell toolbar.', inputSchema: { id: z.string(), cell_type: z.enum(['code', 'sql', 'mojo', 'markdown', 'raw']), allow_unrecoverable: z.boolean().optional(), ...notebookParam } }, async ({ id, cell_type, allow_unrecoverable, notebook }, extra: ToolExtra) => { const target = targetOf(extra, notebook); const res = resolveOne(target, id, extra?.sessionId); if ('error' in res) return res.error; const r = svc.setType(res.id, cell_type, target, { allowUnrecoverable: allow_unrecoverable === true }); if (r.ok) return text(r); // Both refusals ride `refused`, and they are different facts: the .py-notebook one
		// carries its whole message there, the unrecoverable-outputs one carries a CODE and
		// its cause + remedy on `reason` (the `delete_cells` split, for the same reason).
		if (r.refused === 'outputs_unrecoverable') return notFound((r as { reason: string }).reason); return notFound(r.refused ?? `cell ${id} not found`); });
	...
	server.registerTool('run_cells', { description: 'Run several cells in order, each waiting its turn in your notebook\'s kernel queue. Returns a COMPACT batch summary {ran, errored, results}, one record per cell. An OK cell is a status line only — {id, run_status, has_output, has_image, + stale fields if still stale} — its output is one get_full_output(id) away. A batch never inlines figures (a huge token bill across N cells): has_image:true flags a cell that DREW one. An ERRORED cell carries its {ename, evalue, traceback} in full (capped, library frames elided; whole stack via get_full_output(id, size:"full")), so a batch failure is actionable without a second call. Stops at the first cell whose queued run an interrupt/restart cancelled (status "cancelled") — the rest would run against a namespace their predecessors never populated. A CHAT cell is skipped (reported as status "skipped" with its reason), never run: a chat run is a billed model turn that holds the queue slot until it answers - run one deliberately with run_cell.', inputSchema: { ids: z.array(z.string()), ...notebookParam } }, async ({ ids, notebook }, extra: ToolExtra) => { const target = targetOf(extra, notebook); const res = resolveMany(target, ids, extra?.sessionId); if ('error' in res) return res.error; return text(await withProgress(extra, () => svc.runCells(res.ids, target))); });
	server.registerTool('run_all', { description: 'Run all code cells in document order. Returns the same compact {ran, errored, results} batch summary as run_cells (OK cells as status lines with output one get_full_output away; errored cells with full traceback), and the same queueing + cancellation semantics. A CHAT cell is skipped (reported as status "skipped" with its reason), never run: a chat run is a billed model turn that holds the queue slot until it answers - run one deliberately with run_cell.', inputSchema: { ...notebookParam } }, async ({ notebook }, extra: ToolExtra) => text(await withProgress(extra, () => svc.runAll(targetOf(extra, notebook)))));
	...
	server.registerTool('run_range', { description: 'Run code cells in the inclusive range from one cell to another. Returns the same compact {ran, errored, results} batch summary as run_cells (OK cells as status lines with output one get_full_output away; errored cells with full traceback), and the same queueing + cancellation semantics. A CHAT cell is skipped (reported as status "skipped" with its reason), never run: a chat run is a billed model turn that holds the queue slot until it answers - run one deliberately with run_cell.', inputSchema: { from_id: z.string(), to_id: z.string(), ...notebookParam } }, async ({ from_id, to_id, notebook }, extra: ToolExtra) => { const target = targetOf(extra, notebook); const rf = resolveOne(target, from_id, extra?.sessionId); if ('error' in rf) return rf.error; const rt = resolveOne(target, to_id, extra?.sessionId); if ('error' in rt) return rt.error; return text(await withProgress(extra, () => svc.runRange(rf.id, rt.id, target))); });

	// --- databricks (reconnect + gated connect; compute lifecycle stays human-only) ---
	server.registerTool('databricks_status', { description: 'Whether a Databricks Connect session is LIVE in YOUR working notebook\'s kernel, and against which profile/cluster/host (each notebook has its own kernel and its own session). connected:true means `spark` (a Spark session on that cluster) and `w` (a databricks.sdk WorkspaceClient) are bound AND verified reachable - use them directly, never write connection boilerplate. Liveness uses a cached `SELECT 1` (skipped while the kernel is busy) plus the client\'s is_closed flag, so a session that expired server-side or was closed locally is caught even though `spark` is still bound. On expiry Cellar AUTO-RECONNECTS: connected:true with reconnected:true (re-run whatever failed with SESSION_CLOSED). If that fails you get connected:false with expired:true - call databricks_reconnect (it also repairs a dropped kernel socket the probe cannot get through) or ask the user. reauth_required:true overrides that: the named ~/.databrickscfg profile\'s saved sign-in EXPIRED, so no reconnect can succeed - relay the exact reauth_command (`databricks auth login --profile <name>`) for the user to run in a terminal, and do NOT use the sidebar sign-in (it cannot refresh a CLI-managed profile). liveness_unverified:true means liveness could not be confirmed (kernel busy, transient error), NOT that it is dead. connected:false without expired = no session at all: ask the user, or databricks_connect a chosen cluster. dbutils_widgets_warning appears only when set, and is orthogonal to the connection: this kernel\'s SDK-import dbutils (`from databricks.sdk.runtime import dbutils`) is NOT Cellar\'s widgets shim, so parameter values the user enters are discarded whenever a cell re-declares the widget - the controls still render, so nothing on screen shows it. Relay it to the user; a kernel restart rebinds it. Also carries runtime:{advertised, version, forced_by_env} - whether this kernel sets DATABRICKS_RUNTIME_VERSION, i.e. whether IS_DATABRICKS reads true (see databricks_runtime); it is independent of connected. Never boots a kernel. kernel_state carries this same block; get_notebook_map carries connected plus that runtime block.', inputSchema: { ...notebookParam } }, async ({ notebook }, extra: ToolExtra) => text(await svc.databricks.status(targetOf(extra, notebook))));
	...
	server.registerTool('databricks_runtime', { description: 'Advertise a Databricks runtime, applied to YOUR working notebook\'s kernel. The preference is per WORKSPACE: it also governs what every other connected notebook advertises at its next kernel start, though only this one restarts now. It sets DATABRICKS_RUNTIME_VERSION, the variable notebook code\'s IS_DATABRICKS = os.getenv(...) is not None gate reads at import time. True routes parameter handling to dbutils.widgets (real ipywidgets Cellar renders) instead of the local CLI parser (tyro/argparse over sys.argv) that branch falls back to and that cannot work inside a kernel. It is not only your own gate: mlflow\'s is_in_databricks_runtime() reads the same variable, which is why this is opt-in and off by default. Applying it RESTARTS the kernel: every variable is CLEARED and this notebook\'s queued runs are DROPPED - kernel_restarted/namespace_cleared say whether that happened, so re-run your cells when true, and check databricks_status before a cell uses `spark` (the session is rebuilt in the background). It restarts ONLY when the state changes: re-enabling what is already advertised is a no-op, and with no kernel the preference applies at the next start. With NO Databricks session it stores the preference and advertises nothing (scoped to a connected notebook) - connect first, then call again. `version` sets the advertised string (default a recent LTS line), never checked against a real runtime. Read the current state from the runtime block on databricks_status / kernel_state / get_notebook_map. Errors: runtime_env_forced (CELLAR_DATABRICKS_RUNTIME holds the decision; nothing here can move it), runtime_restart_failed (it carries the same pair - namespace_cleared:true there still means your variables are gone).', inputSchema: { enable: z.boolean(), version: z.string().optional(), ...notebookParam } }, async ({ enable, version, notebook }, extra: ToolExtra) => databricksTool(() => withProgress(extra, () => svc.databricks.setRuntime({ enable, version, nb: targetOf(extra, notebook) }))));
