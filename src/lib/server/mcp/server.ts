 ...
 */
const IMAGE_DOC = ` FIGURES COME BACK AS IMAGES YOU CAN SEE: an image output (a matplotlib/plotly figure) is returned as a real image content block, not a text placeholder — so LOOK at the chart you just drew (check the labels, the ticks, whether the data is what you meant) instead of saving it to a file to read it back. Bounded so it stays cheap: an oversized raster is downscaled to ~768px on the longest edge, at most 4 images ride in an automatic RUN result (any others are listed in images_omitted — get_full_output(id) is an explicit per-cell request, so it is NOT capped and returns every image on the cell), and a format that is not inlined (image/svg+xml, image/webp) keeps its text placeholder. Every output still appears in \`outputs\` with its [image/png, WxH, KB] placeholder, so you can see where each figure sat; \`images\` names each one's output_index. get_full_output(id, size:"full") returns the ORIGINAL full-resolution bytes when you need pixel detail.`;
