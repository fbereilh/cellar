...
function registerTools(server: McpServer) {
	...
	server.registerTool('read_cell', { description: 'Read one cell by UUID (source + summarized outputs). ran_this_session/run_status tell you whether the outputs came from the CURRENT kernel session (ok_session) or are saved leftovers from a previous one (ok_persisted). run_status error_kernel_unavailable means the kernel could not be reached; that failure is LIVE, not leftover. stale_state/stale say whether a dependency changed since this cell ran (stale ⇒ re-run before trusting the output).', inputSchema: { id: z.string(), ...notebookParam } }, async ({ id, notebook }, extra: ToolExtra) => { const target = targetOf(extra, notebook); const res = resolveOne(target, id); if ('error' in res) return res.error; const r = await svc.readCell(res.id, target); return r ? text(r) : notFound(`cell ${id} not found or hidden`); });
	server.registerTool('read_cells', { description: 'Read multiple cells by UUID (same per-cell ran_this_session/run_status/stale semantics as read_cell).', inputSchema: { ids: z.array(z.string()), ...notebookParam } }, async ({ ids, notebook }, extra: ToolExtra) => { const target = targetOf(extra, notebook); const res = resolveMany(target, ids); if ('error' in res) return res.error; return text(await svc.readCells(res.ids, target)); });
	...
	server.registerTool('read_section', { description: 'Read all cells under a markdown header (until the next same-or-higher header).', inputSchema: { header_id: z.string(), ...notebookParam } }, async ({ header_id, notebook }, extra: ToolExtra) => { const target = targetOf(extra, notebook); const res = resolveOne(target, header_id); if ('error' in res) return res.error; const r = await svc.readSection(res.id, target); return r ? text(r) : notFound(`${header_id} is not a visible header cell`); });
