...
export function registerTools(server: McpServer) {
	...
	server.registerTool('set_cell_export', { description: 'Mark or unmark ONE OR SEVERAL cells for export to the nbdev-style `.py` module (metadata.cellar.export): export:true marks, false unmarks. Only PYTHON code cells can be exported - a markdown or SQL cell is refused by id, not silently ignored. Nothing is written unless every id resolves. Regenerates the `.py` when a target is set (see set_export_target) AND a cell stays marked - unmarking the LAST one leaves the old module on disk untouched (module.regenerated:false). Refused on a `.py` text notebook (it stores no cell metadata). Returns {ok, exported, count, export_target}; exported = the addressed cells that now carry the REQUESTED value, so on export:false they are now OUT of the module.', inputSchema: { ids: z.array(z.string()), export: z.boolean(), ...notebookParam } }, async ({ ids, export: exported, notebook }, extra: ToolExtra) => {
		const target = targetOf(extra, notebook);
		const res = resolveMany(target, ids, extra?.sessionId);
		if ('error' in res) return res.error;
	...
	server.registerTool('set_export_target', { description: 'Set (or clear) your working notebook\'s EXPORT TARGET — the nbdev-style `#|default_exp` module: the workspace-relative `.py` file the cells marked for export (metadata.cellar.export) are written to. path:"lib/foo.py" sets it; path:null or "" clears it. Persisted in the notebook metadata and regenerates the `.py` immediately when at least one cell is marked for export (set_cell_export) - with none marked, or on a clear, a previously generated module is left on disk as it was. No cell source is touched. Refused on a `.py` text notebook, which stores no such metadata and generates no module — the same refusal as its pair set_cell_export. Returns {export_target}, the same value get_notebook_map\'s `display` block reports.', inputSchema: { path: z.string().nullable(), ...notebookParam } }, async ({ path, notebook }, extra: ToolExtra) => {
