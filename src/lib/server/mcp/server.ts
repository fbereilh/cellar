...
export function registerTools(server: McpServer) {
	...
	server.registerTool('clear_outputs', { description: 'Clear the saved outputs of ONE OR SEVERAL cells by handle; OMIT ids to clear EVERY cell (an empty ids array is an error, never a clear-all). Nothing is cleared unless every id resolves. The pre-clear checkpoint is taken when due (throttled) and keeps no outputs for a big notebook, so undo may not bring them back - `undo` says so when it cannot. Display only: run_status/ran_this_session/staleness unchanged (they read the run stamp, not outputs); only has_output flips. A cell with no outputs is a no-op; one whose run is in flight is SKIPPED (a live run would outrun the report) and named in `skipped`. Returns {ok, cleared:[ids] (those that HAD outputs), count, skipped?, undo?}.', inputSchema: { ids: z.array(z.string()).optional(), ...notebookParam } }, async ({ ids, notebook }, extra: ToolExtra) => {
		const target = targetOf(extra, notebook);
		let full = ids;
	...
	server.registerTool('set_cell_export', { description: 'Mark or unmark ONE OR SEVERAL cells for export to the nbdev-style `.py` module (metadata.cellar.export). Only PYTHON code cells can be exported - markdown/SQL are refused by id. Nothing is written unless every id resolves. Regenerates the `.py` when a target is set (set_export_target) AND a cell stays marked; module says when it was NOT written (nothing marked, write failed, wrote nothing) or WAS but will not import, and why. Refused on a `.py` text notebook (no cell metadata). Returns {ok, cells, count, export_target}: cells = the addressed cells now carrying the REQUESTED value (on export:false, those now OUT of the module); export_target = where they land, #|default_exp included.', inputSchema: { ids: z.array(z.string()), export: z.boolean(), ...notebookParam } }, async ({ ids, export: exported, notebook }, extra: ToolExtra) => {
		const target = targetOf(extra, notebook);
		const res = resolveMany(target, ids, extra?.sessionId);
		if ('error' in res) return res.error;
