...
export function registerTools(server: McpServer) {
	...
	server.registerTool('set_cell_export', { description: 'Mark or unmark ONE OR SEVERAL cells for export to the nbdev-style `.py` module (metadata.cellar.export). Only PYTHON code cells can be exported - markdown/SQL are refused by id, not silently ignored. Nothing is written unless every id resolves. Regenerates the `.py` when a target is set (see set_export_target) AND a cell stays marked - unmarking the LAST one leaves the old module as it was (module.regenerated:false). Refused on a `.py` text notebook (no cell metadata). Returns {ok, exported, count, export_target}: exported = the addressed cells now carrying the REQUESTED value (on export:false, the ones now OUT of the module); export_target = where they land, #|default_exp included.', inputSchema: { ids: z.array(z.string()), export: z.boolean(), ...notebookParam } }, async ({ ids, export: exported, notebook }, extra: ToolExtra) => {
		const target = targetOf(extra, notebook);
		const res = resolveMany(target, ids, extra?.sessionId);
		if ('error' in res) return res.error;
	...
	server.registerTool('set_export_target', { description: 'Set (or clear) your working notebook\'s EXPORT TARGET — the nbdev-style `#|default_exp` module: the workspace-relative `.py` file the cells marked for export (metadata.cellar.export) are written to. path:"lib/foo.py" sets it; path:null or "" clears the SETTING (a `#|default_exp` directive written in a cell is not cleared by this). Persisted in the notebook metadata and regenerates the `.py` when at least one cell is marked for export (set_cell_export) - with none marked, a previously generated module is left on disk as it was. No cell source is touched. Refused on a `.py` text notebook, which stores no such metadata and generates no module — the same refusal as its pair set_cell_export. Returns {export_target}: the EFFECTIVE target, a cell directive included and then flagged export_target_source, and the same value get_notebook_map\'s `display` block reports.', inputSchema: { path: z.string().nullable(), ...notebookParam } }, async ({ path, notebook }, extra: ToolExtra) => {
