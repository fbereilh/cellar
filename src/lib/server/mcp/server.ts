...
3. KERNEL_STATE IS THE LIVE TRUTH; SAVED OUTPUTS ARE NOT. A cell's outputs are
   ...
   says what is actually defined right now. get_notebook_map and read_cell make the
   ...
   (get_notebook_map and kernel_state flag exactly what is stale.)
...
function registerTools(server: McpServer) {
	...
	server.registerTool('read_cell', { description: 'Read one cell by UUID (source + summarized outputs). ran_this_session/run_status tell you whether the outputs came from the CURRENT kernel session (ok_session) or are saved leftovers from a previous one (ok_persisted). run_status error_kernel_unavailable means the kernel could not be reached; that failure is LIVE, not leftover. stale_state/stale say whether a dependency changed since this cell ran (stale ⇒ re-run before trusting the output).', inputSchema: { id: z.string(), ...notebookParam } }, async ({ id, notebook }, extra: ToolExtra) => { const r = await svc.readCell(id, targetOf(extra, notebook)); return r ? text(r) : notFound(`cell ${id} not found or hidden`); });
	server.registerTool('read_cells', { description: 'Read multiple cells by UUID (same per-cell ran_this_session/run_status/stale semantics as read_cell).', inputSchema: { ids: z.array(z.string()), ...notebookParam } }, async ({ ids, notebook }, extra: ToolExtra) => text(await svc.readCells(ids, targetOf(extra, notebook))));
	...
	server.registerTool('read_section', { description: 'Read all cells under a markdown header (until the next same-or-higher header).', inputSchema: { header_id: z.string(), ...notebookParam } }, async ({ header_id, notebook }, extra: ToolExtra) => { const r = await svc.readSection(header_id, targetOf(extra, notebook)); return r ? text(r) : notFound(`${header_id} is not a visible header cell`); });
	server.registerTool('search_cells', { description: 'Search cells; returns ids + snippets. Snippet fields carry the same ran_this_session/run_status semantics as read_cell/get_notebook_map (ran_this_session:false ⇒ the output is a leftover from a previous session; kernel_state is the live truth).', inputSchema: { query: z.string(), in: z.enum(['input', 'output', 'both']).optional(), ...notebookParam } }, async ({ query, in: where, notebook }, extra: ToolExtra) => text(svc.searchCells(query, where ?? 'both', targetOf(extra, notebook))));
	server.registerTool('get_errors', { description: 'List cells whose latest output is an error (ename/evalue/traceback). Each entry carries the same run_status/ran_this_session semantics as read_cell/get_notebook_map: ran_this_session:false errors are leftovers from a previous session — EXCEPT kernel_unavailable:true (run_status error_kernel_unavailable), a LIVE kernel-down failure to fix, not ignore as stale.', inputSchema: { ...notebookParam } }, async ({ notebook }, extra: ToolExtra) => text(svc.getErrors(targetOf(extra, notebook))));
	server.registerTool('get_full_output', { description: 'Fuller cell outputs. Medium-capped by default; size=full returns everything. Images passed through. Output fields carry the same ran_this_session/run_status semantics as read_cell/get_notebook_map (ran_this_session:false ⇒ leftover from a previous session; kernel_state is the live truth).', inputSchema: { id: z.string(), size: z.enum(['medium', 'full']).optional(), ...notebookParam } }, async ({ id, size, notebook }, extra: ToolExtra) => {
		...
		const content: Array<TextBlock | ImageBlock> = [{ type: 'text', text: JSON.stringify({ id: r.id, size: r.size, ran_this_session: r.ran_this_session, outputs: r.outputs.map((o) => (o.data ? { type: o.type, image: o.image } : o)) }) }];
	...
	server.registerTool('delete_cell', { description: 'Delete a cell.', inputSchema: { id: z.string(), ...notebookParam } }, async ({ id, notebook }, extra: ToolExtra) => (svc.removeCell(id, targetOf(extra, notebook)) ? text({ ok: true }) : notFound(`cell ${id} not found`)));
	server.registerTool('move_cell', { description: 'Move a cell to an absolute index.', inputSchema: { id: z.string(), position: z.number().int(), ...notebookParam } }, async ({ id, position, notebook }, extra: ToolExtra) => (svc.moveCell(id, position, targetOf(extra, notebook)) ? text({ ok: true }) : notFound(`cell ${id} not found`)));
	...
	server.registerTool('set_cell_visibility', { description: 'Show/hide a cell from the agent (cellar.hidden_from_agent).', inputSchema: { id: z.string(), hidden: z.boolean(), ...notebookParam } }, async ({ id, hidden, notebook }, extra: ToolExtra) => (svc.setCellVisibility(id, hidden, targetOf(extra, notebook)) ? text({ ok: true, id, hidden }) : notFound(`cell ${id} not found`)));
	...
	server.registerTool('run_cell', { description: 'Run one cell by UUID. Running a markdown cell RENDERS it (no code executes) and returns status "rendered"; use this (or add_and_run) so markdown shows rendered rather than raw source. Your notebook\'s kernel runs one cell at a time: if IT is busy your run is QUEUED (never dropped) and this call waits its turn, then returns the real outputs annotated queued:true + queue_position + waited_ms. A run in another notebook never queues this one (parallel kernels). If that cell is ALREADY queued (or running) it is not enqueued twice - the call returns immediately with status "queued" (or "running") and its queue_position; a pending run has its source refreshed to the current one. status "cancelled" means an interrupt/restart dropped the queued run before it started; nothing executed. See run_queue.', inputSchema: { id: z.string(), ...notebookParam } }, async ({ id, notebook }, extra: ToolExtra) => { const r = await withProgress(extra, () => svc.runCell(id, targetOf(extra, notebook))); return r ? text(r) : notFound(`cell ${id} not found`); });
	server.registerTool('run_cells', { description: 'Run multiple cells in order, each waiting its turn in the kernel queue. Stops at the first cell whose queued run an interrupt/restart cancelled (status "cancelled") - the rest would run against a namespace their predecessors never populated.', inputSchema: { ids: z.array(z.string()), ...notebookParam } }, async ({ ids, notebook }, extra: ToolExtra) => text(await withProgress(extra, () => svc.runCells(ids, targetOf(extra, notebook)))));
	server.registerTool('run_all', { description: 'Run all code cells in document order (same queueing + cancellation semantics as run_cells).', inputSchema: { ...notebookParam } }, async ({ notebook }, extra: ToolExtra) => text(await withProgress(extra, () => svc.runAll(targetOf(extra, notebook)))));
	server.registerTool('run_stale', { description: 'Re-run every STALE code cell (a cell that ran this session but whose inputs changed since) in dependency order, bringing the notebook back in sync with its current code. Returns {ran:[ids], results}. Use this after editing an upstream cell instead of hunting down every downstream cell by hand; see stale_cells in kernel_state / stale_state in get_notebook_map for what is stale. Same queueing + cancellation semantics as run_cells.', inputSchema: { ...notebookParam } }, async ({ notebook }, extra: ToolExtra) => text(await withProgress(extra, () => svc.runStale(targetOf(extra, notebook)))));
	server.registerTool('run_range', { description: 'Run code cells in the inclusive range from one cell to another (same queueing + cancellation semantics as run_cells).', inputSchema: { from_id: z.string(), to_id: z.string(), ...notebookParam } }, async ({ from_id, to_id, notebook }, extra: ToolExtra) => text(await withProgress(extra, () => svc.runRange(from_id, to_id, targetOf(extra, notebook)))));
