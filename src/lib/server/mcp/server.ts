...
8. ONE KERNEL PER NOTEBOOK — ISOLATED NAMESPACES, PARALLEL EXECUTION. Each
   ...
   your kernel tools act on YOUR working notebook and nothing else. Concretely:
     ...
     - Runs serialize only within a notebook. Your notebook's kernel runs one cell
       ...
       An interrupt always ENDS the run, but read its 'stopped' field: "forced" means
       the kernel never answered, so it may still be executing that code even
...
export function registerTools(server: McpServer) {
	...
	server.registerTool('interrupt_kernel', { description: 'Interrupt YOUR working notebook\'s running kernel and drop its queued runs (stop means stop). The run ALWAYS ends: `stopped` says how. "kernel" = it surrendered, the normal case. "forced" = it did not within the grace window, so Cellar stopped waiting and freed the notebook - the namespace is intact and your cells run again, but THE KERNEL MAY STILL BE EXECUTING that code, so relay that and use restart_kernel if you need certainty. Also "chat" (a chat run was aborted), "idle" (nothing was running) and "no_kernel". Affects only your notebook\'s kernel; other notebooks are untouched.', inputSchema: { ...notebookParam } }, async ({ notebook }, extra: ToolExtra) => text(await svc.kernel.interrupt(targetOf(extra, notebook))));
