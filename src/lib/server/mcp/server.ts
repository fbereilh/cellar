...
8. ONE KERNEL, ONE RUN AT A TIME. Every open notebook — and every agent working
   in this Cellar — shares a single kernel, and a human (or another agent on a
   different notebook) may be running a cell in it right now. Your run is never
   dropped: it joins a FIFO queue, so run_cell simply takes longer to return when
   the kernel is busy (its result then carries queued:true and the position it
   waited at). A run of yours waiting behind another notebook's cell is normal and
   expected when several agents share the kernel. run_queue shows what is running
   and what is waiting (each entry names its notebook). Do not interrupt or restart
   the kernel just because a run is slow to come back — that also drops every
   queued run, the user's and other agents' included. Note the kernel namespace is
   shared too: because it is one process, variables one notebook defines are
   visible to code run from another. Keep each notebook's story self-contained
   rather than relying on another notebook having run first.
