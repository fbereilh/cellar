...
export function registerTools(server: McpServer) {
	...
	server.registerTool('delete_cells', { description: 'Delete ONE OR SEVERAL cells by handle in one call (a single-element ids array deletes one cell). Nothing is deleted unless every id resolves, and the whole batch is one checkpoint, never throttled away - undo restores the cells WITH their outputs. If that checkpoint cannot store the outputs (a full disk) the call is REFUSED before deleting anything; allow_unrecoverable:true proceeds knowingly. Deleting EVERY cell is refused - a notebook always keeps at least one. Returns {ok, deleted:[ids], count, undo?}.', inputSchema: { ids: z.array(z.string()), allow_unrecoverable: z.boolean().optional(), ...notebookParam } }, async ({ ids, allow_unrecoverable, notebook }, extra: ToolExtra) => {
		const target = targetOf(extra, notebook);
	...
	server.registerTool('set_cell_type', { description: 'Set a cell type to code, sql, mojo, markdown, or raw. sql tags the code cell as a SQL query (runs against the connected Databricks spark session); mojo tags it as Mojo (doctrine clause 12); code reverts it to Python. raw is verbatim text (frontmatter/directives): never executed, never rendered. raw, mojo and chat are all refused on a .py text notebook, which cannot store the tag. Converting away from code drops that cell\'s outputs and imports role; that conversion takes a checkpoint that stores them, never throttled away, so undo brings them back - and is REFUSED before converting if that checkpoint cannot store them (a full disk), which allow_unrecoverable:true proceeds past knowingly. Its nbdev export mark is KEPT: a mark the new type cannot honour strands rather than being silently deleted, and is cleared from the cell toolbar.', inputSchema: { id: z.string(), cell_type: z.enum(['code', 'sql', 'mojo', 'markdown', 'raw']), allow_unrecoverable: z.boolean().optional(), ...notebookParam } }, async ({ id, cell_type, allow_unrecoverable, notebook }, extra: ToolExtra) => { const target = targetOf(extra, notebook); const res = resolveOne(target, id, extra?.sessionId); if ('error' in res) return res.error; const r = svc.setType(res.id, cell_type, target, { allowUnrecoverable: allow_unrecoverable === true }); if (r.ok) return text({ ok: true }); // Both refusals ride `refused`, and they are different facts: the .py-notebook one
