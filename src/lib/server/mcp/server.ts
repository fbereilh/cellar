...
function registerTools(server: McpServer) {
	...
	server.registerTool('search_cells', { description: 'Search cells; returns ids + snippets. Snippet fields carry the same ran_this_session/run_status semantics as read_cells/get_notebook_map (ran_this_session:false ⇒ the output is a leftover from a previous session; kernel_state is the live truth).', inputSchema: { query: z.string(), in: z.enum(['input', 'output', 'both']).optional(), ...notebookParam } }, async ({ query, in: where, notebook }, extra: ToolExtra) => text(svc.searchCells(query, where ?? 'both', targetOf(extra, notebook))));
