...
3. KERNEL_STATE IS THE LIVE TRUTH; SAVED OUTPUTS ARE NOT. A cell's outputs are
   ...
   inspects the kernel namespace to check it. A cell that reads a name it also
   rebinds (df = f(df), count += 1) records no dependency at all, so editing its
   upstream leaves it reported FRESH while its output is already out of date; the
   same goes for names reached through exec/globals(). "stale" always means re-run
   it; "fresh" means nothing was detected, not that it was verified. When you edit
   a cell that defines a name, re-run the cells below it that use it even if they
   are not flagged - or restart_kernel + run_all when you need certainty.
...
5. STRUCTURE WITH MARKDOWN. Use markdown header cells to divide the notebook into
   ...
   your markdown and your outputs to carry the whole story when it is on.
...
function registerTools(server: McpServer) {
	...
	server.registerTool('set_report_view', { description: 'Turn the notebook-wide report view on/off for your working notebook (enabled:true = report view ON: every code cell renders its OUTPUT only, with its code input hidden, so a human reads results and markdown without the code; enabled:false = code shown again). Display-only - no source is touched and cells still run normally. A per-cell override (set by the user in the UI) can still show an individual cell\'s code. Returns the resulting report_view state; get_notebook_map reports whether it is on.', inputSchema: { enabled: z.boolean(), ...notebookParam } }, async ({ enabled, notebook }, extra: ToolExtra) => text(svc.setReportView(enabled, targetOf(extra, notebook))));
