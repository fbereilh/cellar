...
export function registerTools(server: McpServer) {
	...
	server.registerTool('set_export_target', { description: 'Set (or clear) your working notebook\'s EXPORT TARGET — the nbdev-style `#|default_exp` module: the workspace-relative `.py` file the cells marked for export (metadata.cellar.export) are written to. path:"lib/foo.py" sets it; path:null or "" clears the SETTING (a `#|default_exp` directive written in a cell is not cleared by this). Persisted in the notebook metadata and regenerates the `.py` when at least one cell is marked for export (set_cell_export); module.regenerated:false says the write FAILED, and why. No cell source is touched. A path outside the workspace is refused, as is a `.py` text notebook, which stores no such metadata — like its pair set_cell_export. Returns {export_target}: the EFFECTIVE target, a cell directive included and then flagged export_target_source, and the same value get_notebook_map\'s `display` block reports.', inputSchema: { path: z.string().nullable(), ...notebookParam } }, async ({ path, notebook }, extra: ToolExtra) => {
