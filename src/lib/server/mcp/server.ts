...
function registerTools(server: McpServer) {
	...
	server.registerTool('get_full_output', { description: 'Fuller cell outputs. Medium-capped by default; size=full returns everything. Images are returned as real MCP image blocks. An oversized image (a retina/high-DPI plot) is DOWNSCALED to ~768px on the longest edge on the default (medium) path to save image tokens - the block carries a `downscaled:{from,to}` note when this happened; call again with size=full to get the ORIGINAL full-resolution image bytes untouched. Output fields carry the same ran_this_session/run_status semantics as read_cell/get_notebook_map (ran_this_session:false ⇒ leftover from a previous session; kernel_state is the live truth).', inputSchema: { id: z.string(), size: z.enum(['medium', 'full']).optional(), ...notebookParam } }, async ({ id, size, notebook }, extra: ToolExtra) => {
