...
function registerTools(server: McpServer) {
	...
	server.registerTool('clear_outputs', { description: 'Clear the saved outputs of ONE OR SEVERAL cells by handle in one call; OMIT ids to clear EVERY cell in the notebook (an empty ids array is an error, never a clear-all). Nothing is cleared unless every id resolves. A pre-clear checkpoint is taken WHEN DUE (they are throttled) and stores no outputs for a big notebook, so undo may not bring them back - `undo` says so when it cannot. Display only: nothing runs, no source changes, and run_status/ran_this_session/staleness are unchanged (they come from the run stamp, not from having outputs) - only has_output flips. A cell with no outputs is a no-op; a cell whose run is in flight is SKIPPED (the run would write its outputs straight back) and named in `skipped`. Returns {ok, cleared:[ids] (the cells that HAD outputs), count, skipped?, undo?}.', inputSchema: { ids: z.array(z.string()).optional(), ...notebookParam } }, async ({ ids, notebook }, extra: ToolExtra) => {
		const target = targetOf(extra, notebook);
		let full = ids;
