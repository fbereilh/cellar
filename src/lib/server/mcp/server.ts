...
export function registerTools(server: McpServer) {
	...
	server.registerTool('set_export_target', { description: 'Set (or clear) your working notebook\'s EXPORT TARGET: the nbdev `#|default_exp` module — the workspace-relative `.py` file that cells marked for export are written to. path:"lib/foo.py" sets it; path:null or "" clears the SETTING (a `#|default_exp` directive written in a cell is NOT). Persisted in metadata; the `.py` is rewritten while a cell stays marked (set_cell_export), and module.regenerated:false says that write FAILED, and why. Refused: a path outside the workspace or not a `.py` file (the module is WRITTEN there), and a `.py` text notebook, which stores no such metadata. Returns {export_target}, the EFFECTIVE target — a cell directive included, flagged export_target_source.', inputSchema: { path: z.string().nullable(), ...notebookParam } }, async ({ path, notebook }, extra: ToolExtra) => {
